...
`S1-06` introduced persisted risk classification, but the current saved
result is still too opaque: a risk level can exist without a clear saved
explanation for why that classification was chosen. That weakens review,
teaching value, export fidelity, and the long-term usefulness of the
assessment record.

This story adds explicit persisted reasoning to the risk-entry
classification flow. The reasoning belongs with the risk-entry
classification domain and must flow through app-owned mapping into
exports without asking `@vardi/export` to invent prose or expand
...
- Given empty reasoning and a product rule that requires it, the UI
  blocks save, completion, or export as appropriate with clear feedback.
